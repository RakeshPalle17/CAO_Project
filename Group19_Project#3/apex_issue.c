...
updateLSQentrywithForwardedBus(APEX_CPU *cpu)
{
    for (int i = cpu->LSQ_head; i <= cpu->LSQ_tail; ++i)
    {
        ...
        if (!cpu->lsq[i].LorS_bit)
        {
            ...
            if (!cpu->lsq[i].src_data_valid && cpu->lsq[i].src_tag == cpu->MAU_frwded_tag)
            {
                cpu->lsq[i].valueToStore = cpu->MAU_frwded_value;
                cpu->lsq[i].src_data_valid = VALID;
            }

            if (!cpu->lsq[i].src_data_valid && cpu->lsq[i].src_tag == cpu->BFU_frwded_tag)
            {
                cpu->lsq[i].valueToStore = cpu->BFU_frwded_value;
                cpu->lsq[i].src_data_valid = VALID;
            }
        }
    }
